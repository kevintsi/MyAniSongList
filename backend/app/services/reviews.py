...
class ReviewService(BaseService[Review, ReviewCreate, ReviewUpdate]):
    ...
    def get_user_review(self, id_music, id_user):
        review = self.db_session.scalars(select(Review).filter(
    ...
    def calculate_note(self, obj: Review):
        music: Music = self.db_session.scalars(
            select(Music).filter(Music.id == obj.music_id)).first()

        total_sum = self.db_session.scalars(select(sqlalchemy.func.sum(
            Review.note_visual+Review.note_music)).filter(Review.music_id == music.id)).first()

        reviews_count = self.db_session.scalars(select(sqlalchemy.func.count(Review.id)).filter(
            Review.music_id == music.id)).first()
    ...
    def get_music_review(self, id_music: int):
        return select(Review).filter(Review.music_id == id_music, Review.description != "")
