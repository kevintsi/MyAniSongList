...
class AnimeTranslation(Base):
    ...
    id_anime = mapped_column(
        ForeignKey("anime.id", ondelete="CASCADE"),
        primary_key=True,
        index=True,
    )
    id_language = mapped_column(
        ForeignKey("supported_language.id", ondelete="CASCADE"),
        primary_key=True,
        index=True,
    )
...
class Language(Base):
    ...
    anime_translations: Mapped[List[AnimeTranslation]] = relationship(
        back_populates="language", cascade="all, delete"
    )
    type_translations: Mapped[List[TypeTranslation]] = relationship(
        back_populates="language", cascade="all, delete"
    )
...
class Music(Base):
    ...
    anime_id = mapped_column(
        ForeignKey("anime.id"), nullable=False, index=True
    )
    ...
    type: Mapped["Type"] = relationship("Type", back_populates="musics")
    reviews: Mapped[List["Review"]] = relationship(
