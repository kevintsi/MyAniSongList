...
class TestMusics:
    ...
    def test_create_music(self, test_app_with_db, get_token_manager):
        ...
        anime = AnimeCreate(
            name="Kimetsu no Yaiba", description="description fr"
        )
        artist = ArtistCreate(name="LiSA", creation_year="2005")
        music = MusicCreate(
            name="GurengezzzzzFAIL",
        ...
        assert (
            response_get_music.json()["name"]
            == response_post_music.json()["name"]
        )
        assert (
            response_get_music.json()["id"] == response_post_music.json()["id"]
        )
    ...
    def test_get_music_by_id(self, test_app_with_db):
        ...
        music = Music(
            ...
            name="GurengezzzzzFAIL",
            avg_note=None,
            poster_img=blob_music.public_url,
            release_date=datetime(2018, 10, 1),
            artists=[
                Artist(
                    id=1,
                    name="LiSA",
                    creation_year="2005",
                    poster_img=blob_artist.public_url,
                )
            ],
            ...
            anime=Anime(
                ...
                poster_img=blob_anime.public_url,
            ),
        )
    ...
    def test_update_music(self, test_app_with_db, get_token_manager):
        ...
        assert (
            response_update.json()["id_video"]
            == response_get.json()["id_video"]
        )
    ...
    def test_get_musics_by_id_anime(self, test_app_with_db, get_token_manager):
        ...
        music = MusicCreate(
            name="Koi kogare",
            release_date=datetime(2023, 10, 1),
            artists=[2],
            type_id=2,
            id_video="x45dsF",
            anime_id=1,
        ...
        file_music = open(
            "/usr/src/app/tests/images_test/koi_kogare_ending_kny.jpg",
            mode="rb",
        )
