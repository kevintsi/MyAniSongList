...
class TestArtists:
    ...
    def test_create_artist(self, test_app_with_db, get_token_manager):
        artists = [{"artist": ArtistCreate(name="FLOW", creation_year="1998"), "poster_img": "flow_band.jpg"},
                   {"artist": ArtistCreate(
                       name="MAN WITH A MISSION", creation_year="2000"), "poster_img": "man_with_a_mission.jpg"},
                   {"artist": ArtistCreate(name="Mile", creation_year="2003"), "poster_img": "milet.jpg"}, {"artist": ArtistCreate(name="LiSA", creation_year="2005"), "poster_img": "LiSA.jpg"}]
        for artist in artists:
            with open(f"/usr/src/app/tests/images_test/{artist['poster_img']}", "rb") as f:
                response = test_app_with_db.post(f"{self.ENDPOINT_BASE}/add", data={"artist": artist["artist"].json()}, headers={
                    "Authorization": f"Bearer {get_token_manager}"
                }, files={"poster_img": (os.path.basename(f.name), f, "image/jpeg")})

                blob = bucket.blob(
                    f"artist_poster_images/{artist['poster_img']}")
                blob.make_public()

                assert response.status_code == 201
                assert response.json()["name"] == artist["artist"].name
                assert response.json()[
                    "creation_year"] == artist["artist"].creation_year
    ...
    def test_get_artist_by_id(self, test_app_with_db):
        ...
        blob.make_public()
        artist = Artist(id=1, name="FLOW", creation_year="1998",
        ...
        assert response.json()[
            "creation_year"] == artist.creation_year
    ...
    def test_update_artist(self, test_app_with_db, get_token_manager):
        ...
        response_put = test_app_with_db.put(
            f"{self.ENDPOINT_BASE}/update/3", data={"artist": artist.json()}, headers={
                "Authorization": f"Bearer {get_token_manager}"
            })
