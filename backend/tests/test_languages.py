...
class TestLanguages():
    ...
    def test_create_language(self, test_app_with_db, get_token_manager):
        languages = [LanguageCreate(code="fr"), LanguageCreate(
            code="en"), LanguageCreate(code="ch"), LanguageCreate(code="it")]
        for lang in languages:
            response = test_app_with_db.post(f"{self.ENDPOINT_BASE}/add", json=lang.dict(), headers={
    ...
    def test_update_language(self, test_app_with_db, get_token_manager):
        ...
        response_put = test_app_with_db.put(
            f"{self.ENDPOINT_BASE}/update/3", json=lang.dict(), headers={
                "Authorization": f"Bearer {get_token_manager}"
            })
    ...
    def test_delete_language(self, test_app_with_db, get_token_manager):
        response = test_app_with_db.delete(f"{self.ENDPOINT_BASE}/delete/4", headers={
            "Authorization": f"Bearer {get_token_manager}"
        })
        assert response.status_code == 200
