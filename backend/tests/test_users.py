...
class TestUsers:
    ...
    def test_create_user(self, test_app_with_db):
        user = UserCreate(
            email="test2@gmail.com", username="test2", password="motdepasse"
        )
    ...
    def test_update_profile(self, test_app_with_db):
        ...
        assert response_update.json() == User(
            id=response_get.json()["id"],
            email=response_get.json()["email"],
            username=response_get.json()["username"],
    ...
    def test_search_user(self, test_app_with_db):
        new_users = [
            ...
            UserCreate(
                email="arthur.johns@gmail.com",
                username="arthurjohns",
                password="motdepasse",
            ),
        ]
    ...
    def test_get_my_profile(self, test_app_with_db):
        ...
        response_me = test_app_with_db.get(
            f"{self.ENDPOINT_BASE}/me",
            headers={
