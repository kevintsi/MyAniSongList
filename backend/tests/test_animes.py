...
class TestAnimes():
    ...
    def test_create_anime(self, test_app_with_db, get_token_manager):
        ...
        animes = [{"anime": AnimeCreate(name="Naruto", description="description fr naruto"), "poster_img": "naruto_affiche.jpg"},
                  {"anime": AnimeCreate(
                      name="One Pice", description="description fr one piece"), "poster_img": "one_piece.jpg"},
                  {"anime":  AnimeCreate(name="Vinland Saga", description="description fr vinland saga"), "poster_img": "vinland_saga.jpg"}]
        for anime in animes:
            with open(f"/usr/src/app/tests/images_test/{anime['poster_img']}", "rb") as f:
                print("Opening....")
                response = test_app_with_db.post(f"{self.ENDPOINT_BASE}/add", data={"anime": anime["anime"].json()}, headers={
                    "Authorization": f"Bearer {get_token_manager}"
    ...
    def test_get_anime_by_id(self, test_app_with_db):
        blob = bucket.blob(
            f"anime_poster_images/naruto_affiche.jpg")
        blob.make_public()
        anime = Anime(id=1, name="Naruto",
                      description="description fr naruto", poster_img=blob.public_url)
