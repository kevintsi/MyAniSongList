...
class TestTypes:
    ...
    def test_create_type_translation(
        ...
        response = test_app_with_db.post(
            f"{self.ENDPOINT_BASE}/1/add_translation?lang=jp",
            json=type_.dict(),
            headers={"Authorization": f"Bearer {get_token_manager}"},
        )
        assert response.status_code == 201
    ...
    def test_update_type_translation(
        self, test_app_with_db, get_token_manager
    ):
        type = TypeUpdate(name="オープニング")
        response = test_app_with_db.put(
            f"{self.ENDPOINT_BASE}/1/update_translation?lang=jp",
            json=type.dict(),
            headers={"Authorization": f"Bearer {get_token_manager}"},
        )
        assert response.status_code == 200
